...
import (
	...
	omni "github.com/voxgig-sdk/passantenfrequenzen-zuerich-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.PassantenfrequenzenZuerichError); is {
				out := map[string]any{
					"name":    "PassantenfrequenzenZuerichError",
...
func omniCtx(arg any, client *sdk.PassantenfrequenzenZuerichSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.PassantenfrequenzenZuerichSDK); is {
			client = live
