...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("PASSANTENFREQUENZENZUERICH_TEST_LIVE") == "TRUE" ||
		os.Getenv("PASSANTENFREQUENZENZUERICH_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("PASSANTENFREQUENZENZUERICH_TEST_EXPLAIN"); explain != "" {
