...
type PassantenfrequenzenZuerichSDK struct {
	Mode     string
...
func NewPassantenfrequenzenZuerichSDK(options map[string]any) *PassantenfrequenzenZuerichSDK {
	sdk := &PassantenfrequenzenZuerichSDK{
		Mode:     "live",
		Features: []Feature{},
	}
...
func (sdk *PassantenfrequenzenZuerichSDK) OptionsMap() map[string]any {
...
func (sdk *PassantenfrequenzenZuerichSDK) GetUtility() *Utility {
	return CopyUtility(sdk.utility)
}

func (sdk *PassantenfrequenzenZuerichSDK) GetRootCtx() *Context {
	return sdk.rootctx
}

func (sdk *PassantenfrequenzenZuerichSDK) Prepare(fetchargs map[string]any) (map[string]any, error) {
	utility := sdk.utility
...
func (sdk *PassantenfrequenzenZuerichSDK) Direct(fetchargs map[string]any) (map[string]any, error) {
...
func (sdk *PassantenfrequenzenZuerichSDK) opAllowed(op string) bool {
	allowOp, _ := vs.GetPath([]any{"allow", "op"}, sdk.options).(string)
	return strings.Contains(allowOp, op)
}

func (sdk *PassantenfrequenzenZuerichSDK) opDenied(op string) map[string]any {
	allowOp, _ := vs.GetPath([]any{"allow", "op"}, sdk.options).(string)
	return map[string]any{
		"ok": false,
		"err": fmt.Errorf("PassantenfrequenzenZuerichSDK: %s: operation not allowed by"+
...
// gate by passing it.
func (sdk *PassantenfrequenzenZuerichSDK) rawRequest(fetchargs map[string]any) (map[string]any, error) {
	utility := sdk.utility
...
func (sdk *PassantenfrequenzenZuerichSDK) Graphql(
	...
	if 0 < len(errors) {
		...
		res["ok"] = false
		res["err"] = fmt.Errorf("PassantenfrequenzenZuerichSDK: graphql: %s", msg)
		res["graphql"] = errors
	}
...
func TestSDK(testopts map[string]any, sdkopts map[string]any) *PassantenfrequenzenZuerichSDK {
	...
	sdk := NewPassantenfrequenzenZuerichSDK(sdkopts)
	sdk.Mode = "test"
