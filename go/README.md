# PassantenfrequenzenZuerich Golang SDK
...
## Install
```bash
go get github.com/voxgig-sdk/passantenfrequenzen-zuerich-sdk/go@latest
```

The Go module proxy resolves the version from the `go/vX.Y.Z` GitHub
release tag — see [Releases](https://github.com/voxgig-sdk/passantenfrequenzen-zuerich-sdk/releases) for the available versions.
...
```bash
go mod edit -replace github.com/voxgig-sdk/passantenfrequenzen-zuerich-sdk/go=../passantenfrequenzen-zuerich-sdk/go
```
...
func main() {
    ...
    // List frequenzen records — the value is the array of records itself.
    frequenzens, err := client.Frequenzen(nil).List(nil, nil)
    if err != nil {
        panic(err)
    }
    for _, item := range frequenzens.([]any) {
        fmt.Println(item)
    }
}
```
...
### NewPassantenfrequenzenZuerichSDK
...
```go
func TestSDK(testopts map[string]any, sdkopts map[string]any) *PassantenfrequenzenZuerichSDK
```

Creates a test-mode client with mock transport. Both arguments may be `nil`.

### PassantenfrequenzenZuerichSDK methods
...
| `Frequenzen` | `(data map[string]any) PassantenfrequenzenZuerichEntity` | Create a Frequenzen entity instance. |
| `Standorte` | `(data map[string]any) PassantenfrequenzenZuerichEntity` | Create a Standorte entity instance. |

### Entity interface (PassantenfrequenzenZuerichEntity)

All entities implement the `PassantenfrequenzenZuerichEntity` interface.
...
API path: `/dataset/hystreet_fussgaengerfrequenzen/download/hystreet_fussgaengerfrequenzen_seit2021.csv`
...
API path: `/dataset/hystreet_fussgaengerfrequenzen/download/hystreet_locations.json`
...
Create an instance: `frequenzen := client.Frequenzen(nil)`
...
fmt.Println(frequenzens) // the array of records
```


### Standorte

Create an instance: `standorte := client.Standorte(nil)`
...
```go
standortes, err := client.Standorte(nil).List(nil, nil)
if err != nil {
    panic(err)
}
fmt.Println(standortes) // the array of records
```
...
```
github.com/voxgig-sdk/passantenfrequenzen-zuerich-sdk/go/
├── passantenfrequenzen-zuerich.go        # Root package — type aliases and constructors
...
The root package (`github.com/voxgig-sdk/passantenfrequenzen-zuerich-sdk/go`) re-exports everything needed
