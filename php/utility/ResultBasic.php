...
// PassantenfrequenzenZuerich SDK utility: result_basic

class PassantenfrequenzenZuerichResultBasic
{
    public static function call(PassantenfrequenzenZuerichContext $ctx): ?PassantenfrequenzenZuerichResult
    {
        ...
        if ($result && $response) {
            ...
            if ($result->status >= 400) {
                ...
                if ($result->err) {
                    $prev = ($result->err instanceof PassantenfrequenzenZuerichError) ? $result->err->msg : (string)$result->err;
