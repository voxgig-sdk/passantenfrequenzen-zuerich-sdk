...
class PassantenfrequenzenZuerichSDK
{
    public string $mode;
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = PassantenfrequenzenZuerichHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = PassantenfrequenzenZuerichHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!PassantenfrequenzenZuerichFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, PassantenfrequenzenZuerichFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return PassantenfrequenzenZuerichUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = PassantenfrequenzenZuerichHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = PassantenfrequenzenZuerichHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = PassantenfrequenzenZuerichHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new PassantenfrequenzenZuerichError($op . "_allow",
                "PassantenfrequenzenZuerichSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = PassantenfrequenzenZuerichHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new PassantenfrequenzenZuerichError("graphql_error",
                "PassantenfrequenzenZuerichSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    public function Frequenzen($data = null)
    {
        ...
        if ($data === null) {
            if ($this->_frequenzen === null) {
                $this->_frequenzen = new FrequenzenEntity($this, null);
            }
            return $this->_frequenzen;
        }
        return new FrequenzenEntity($this, $data);
    }
    ...
    public function Standorte($data = null)
    {
        ...
        if ($data === null) {
            if ($this->_standorte === null) {
                $this->_standorte = new StandorteEntity($this, null);
            }
            return $this->_standorte;
        }
        return new StandorteEntity($this, $data);
    }
    ...
    public static function test(?array $testopts = null, ?array $sdkopts = null): self
    {
        ...
        $sdk = new PassantenfrequenzenZuerichSDK($sdkopts);
        $sdk->mode = "test";
        return $sdk;
    }
}
