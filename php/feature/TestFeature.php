...
// PassantenfrequenzenZuerich SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class PassantenfrequenzenZuerichTestFeature extends PassantenfrequenzenZuerichBaseFeature
{
    ...
    public function init(PassantenfrequenzenZuerichContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (PassantenfrequenzenZuerichContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = PassantenfrequenzenZuerichParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (PassantenfrequenzenZuerichContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(PassantenfrequenzenZuerichContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = PassantenfrequenzenZuerichParam::call($ctx, $k);
