...
// PassantenfrequenzenZuerich SDK feature test
//
...
class FtCtrl extends PassantenfrequenzenZuerichControl
{
...
class FtClient
{
    ...
    private PassantenfrequenzenZuerichUtility $utility;

    public function __construct(PassantenfrequenzenZuerichUtility $utility, array $options)
    {
    ...
    public function get_utility(): PassantenfrequenzenZuerichUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (PassantenfrequenzenZuerichContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public PassantenfrequenzenZuerichContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (PassantenfrequenzenZuerichContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = PassantenfrequenzenZuerichConfig::make_feature($name);
    ...
    private static function buildUrl(PassantenfrequenzenZuerichSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof PassantenfrequenzenZuerichError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new PassantenfrequenzenZuerichResponse($fetched);
        }
    ...
    private function fail(PassantenfrequenzenZuerichContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (PassantenfrequenzenZuerichContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
