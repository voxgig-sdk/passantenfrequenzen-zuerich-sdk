...
Dieses API ermöglicht den Zugriff auf Daten zu Passantenfrequenzen an verschiedenen Standorten in Zürich.

Learn more about Passantenfrequenzen Zürich at [data.stadt-zuerich.ch/dataset/hystreet_fussgaengerfrequenzen](https://data.stadt-zuerich.ch/dataset/hystreet_fussgaengerfrequenzen).
...
This SDK exposes the API as a small set of **semantic entities** — Frequenzen and Standorte — that you
...
const items = await client.Frequenzen().list()
```
...
```go
client := sdk.Test()
result, err := client.Frequenzen(nil).List(
    nil, nil,
)
```
...
$frequenzens = $client->Frequenzen()->list();
print_r(array_map(fn($item) => $item->data_get(), $frequenzens));
```
