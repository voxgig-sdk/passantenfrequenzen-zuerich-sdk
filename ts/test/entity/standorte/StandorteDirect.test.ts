...
describe('StandorteDirect', async () => {
  ...
  test('direct-list-standorte', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-standorte', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['PASSANTENFREQUENZEN_ZUERICH_TEST_STANDORTE_ENTID']
