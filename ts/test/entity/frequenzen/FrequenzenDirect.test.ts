...
describe('FrequenzenDirect', async () => {
  ...
  test('direct-exists', async () => {
    const sdk = new PassantenfrequenzenZuerichSDK({
  ...
  test('direct-list-frequenzen', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-frequenzen', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['PASSANTENFREQUENZEN_ZUERICH_TEST_FREQUENZEN_ENTID']
