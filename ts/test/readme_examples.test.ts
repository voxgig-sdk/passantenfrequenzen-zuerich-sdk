...
//     (\`new PassantenfrequenzenZuerichSDK(...)\` / \`PassantenfrequenzenZuerichSDK.test(...)\` become
//     \`PassantenfrequenzenZuerichSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'PassantenfrequenzenZuerichSDK'
...
const TEST_SEED = {"entity":{"frequenzen":{"test01":{"id":"test01"}},"standorte":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new PassantenfrequenzenZuerichSDK(...)\` and any
// \`PassantenfrequenzenZuerichSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+PassantenfrequenzenZuerichSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/PassantenfrequenzenZuerichSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
