// Command PassantenfrequenzenZuerich-cli is a CLI + REPL for the PassantenfrequenzenZuerich SDK,
...
const prompt = "passantenfrequenzen-zuerich"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "frequenzen standorte"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      PASSANTENFREQUENZEN_ZUERICH_APIKEY (api key), PASSANTENFREQUENZEN_ZUERICH_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: PASSANTENFREQUENZEN_ZUERICH_APIKEY carries the API key and
	// PASSANTENFREQUENZEN_ZUERICH_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("PASSANTENFREQUENZEN_ZUERICH_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("PASSANTENFREQUENZEN_ZUERICH_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewPassantenfrequenzenZuerichSDK(opts)
