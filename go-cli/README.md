# passantenfrequenzen-zuerich-cli

boru-driven command-line client **and** interactive REPL for the PassantenfrequenzenZuerich
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/passantenfrequenzen-zuerich-cli)
make build

# 2. See usage (words, entities, env vars)
./passantenfrequenzen-zuerich-cli --help

# 3. Provide credentials once, via the environment
export PASSANTENFREQUENZEN_ZUERICH_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./passantenfrequenzen-zuerich-cli list frequenzen
./passantenfrequenzen-zuerich-cli list standorte

# 5. Override the API base URL for a single call
PASSANTENFREQUENZEN_ZUERICH_BASE=https://api.example.com ./passantenfrequenzen-zuerich-cli list frequenzen

# 6. No arguments -> interactive REPL
./passantenfrequenzen-zuerich-cli
passantenfrequenzen-zuerich> list frequenzen
passantenfrequenzen-zuerich> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/passantenfrequenzen-zuerich-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/passantenfrequenzen-zuerich-cli list frequenzen
   ```
...
Run with no arguments to open a REPL (prompt `passantenfrequenzen-zuerich>`). Each line is
evaluated as its own boru expression:

```text
$ ./passantenfrequenzen-zuerich-cli
passantenfrequenzen-zuerich> list frequenzen
passantenfrequenzen-zuerich> /help
passantenfrequenzen-zuerich> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `frequenzen`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/passantenfrequenzen-zuerich-cli`. |
...
(`./passantenfrequenzen-zuerich-cli <expr>`) and interactively (the REPL), and expressions compose the
