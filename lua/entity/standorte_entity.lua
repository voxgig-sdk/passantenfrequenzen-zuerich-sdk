...
local StandorteEntity = {}
StandorteEntity.__index = StandorteEntity


function StandorteEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, StandorteEntity)
  self._name = "standorte"
  self._client = client
...
function StandorteEntity:get_name()
  return self._name
end


function StandorteEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return StandorteEntity.new(self._client, opts)
end
...
function StandorteEntity:mark_deleted()
  self._deleted = true
end


function StandorteEntity:deleted()
  return true == self._deleted
end


function StandorteEntity:data_set(args)
  if args ~= nil then
...
function StandorteEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function StandorteEntity:match_set(args)
  if args ~= nil then
...
function StandorteEntity:match_get()
...
function StandorteEntity:stream(action, args, callopts)
...
---@param reqmatch StandorteListMatch
---@param ctrl? table
---@return Standorte[]
---@return string? err
function StandorteEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function StandorteEntity:_run_op(ctx, post_done)
  local utility = self._utility
