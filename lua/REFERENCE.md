# PassantenfrequenzenZuerich Lua SDK Reference

Complete API reference for the PassantenfrequenzenZuerich Lua SDK.
...
```lua
local results, err = client:Frequenzen(nil):list(nil, nil)
```
...
```lua
local results, err = client:Standorte(nil):list(nil, nil)
```
