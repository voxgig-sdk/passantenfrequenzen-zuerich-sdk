...
require("passantenfrequenzen-zuerich_types")
...
local PassantenfrequenzenZuerichSDK = {}
PassantenfrequenzenZuerichSDK.__index = PassantenfrequenzenZuerichSDK
...
PassantenfrequenzenZuerichSDK._make_feature = _make_feature


function PassantenfrequenzenZuerichSDK.new(options)
  local self = setmetatable({}, PassantenfrequenzenZuerichSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function PassantenfrequenzenZuerichSDK:options_map()
...
function PassantenfrequenzenZuerichSDK:get_utility()
  return Utility.copy(self._utility)
end


function PassantenfrequenzenZuerichSDK:get_root_ctx()
  return self._rootctx
end


function PassantenfrequenzenZuerichSDK:prepare(fetchargs)
...
function PassantenfrequenzenZuerichSDK:direct(fetchargs)
...
function PassantenfrequenzenZuerichSDK:_op_allowed(op)
  local allow = vs.getpath(self.options, "allow.op")
  return type(allow) == "string" and allow:find(op, 1, true) ~= nil
end


function PassantenfrequenzenZuerichSDK:_op_denied(op)
  local allow = vs.getpath(self.options, "allow.op")
  if type(allow) ~= "string" then allow = "" end
  return {
    ok = false,
    err = "PassantenfrequenzenZuerichSDK: " .. op .. ": operation not allowed by" ..
...
-- by passing it.
function PassantenfrequenzenZuerichSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function PassantenfrequenzenZuerichSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "PassantenfrequenzenZuerichSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function PassantenfrequenzenZuerichSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = PassantenfrequenzenZuerichSDK.new(sdkopts)
  sdk.mode = "test"
...
return PassantenfrequenzenZuerichSDK
