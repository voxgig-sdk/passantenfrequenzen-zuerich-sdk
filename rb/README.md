# PassantenfrequenzenZuerich Ruby SDK



The Ruby SDK for the PassantenfrequenzenZuerich API — an entity-oriented client using idiomatic Ruby conventions.

The SDK exposes the API as capitalised, semantic **Entities** — for example `client.Frequenzen` — with named operations (`list`) instead of raw URL paths and query strings. Working with resources and verbs keeps call sites self-describing and reduces cognitive load.
...
```ruby
client = PassantenfrequenzenZuerichSDK.test(testopts, sdkopts)
```
...
| `Frequenzen` | `(data) -> FrequenzenEntity` | Create a Frequenzen entity instance. |
| `Standorte` | `(data) -> StandorteEntity` | Create a Standorte entity instance. |
...
raise a `PassantenfrequenzenZuerichError` (a `StandardError` subclass), so wrap
...
| `count` | `Integer` | Anzahl gezählter Passanten |
...
| `zone` | `Integer` | Zone (1-3 für Bürgersteigseiten/Mitte, 99 für nicht zuordenbar) |
...
```
rb/
├── PassantenfrequenzenZuerich_sdk.rb       -- Main SDK module
...
The main module (`PassantenfrequenzenZuerich_sdk`) exports the SDK class
...
# frequenzen.data_get now returns the frequenzen data from the last list
