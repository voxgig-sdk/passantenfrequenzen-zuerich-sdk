...
class FrequenzenEntity
  ...
  def make
    opts = @_entopts.dup
    FrequenzenEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = PassantenfrequenzenZuerichHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Frequenzen, Hash] the current Frequenzen data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = PassantenfrequenzenZuerichHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Frequenzen fields)
  def match_get
  ...
  #
  # @param reqmatch [FrequenzenListMatch, Hash, nil] match filter (any subset of
  #   Frequenzen fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Frequenzen>, Array] the matching Frequenzen items; raises PassantenfrequenzenZuerichError on failure
