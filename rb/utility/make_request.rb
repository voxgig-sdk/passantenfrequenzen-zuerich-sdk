...
module PassantenfrequenzenZuerichUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = PassantenfrequenzenZuerichResponse.new({})
    ...
    if fetch_err
      ...
      response = PassantenfrequenzenZuerichResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = PassantenfrequenzenZuerichResponse.new(fetched)
    else
