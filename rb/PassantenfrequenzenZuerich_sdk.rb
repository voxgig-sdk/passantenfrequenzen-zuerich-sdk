...
require_relative 'PassantenfrequenzenZuerich_types'
...
class PassantenfrequenzenZuerichSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = PassantenfrequenzenZuerichHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = PassantenfrequenzenZuerichHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, PassantenfrequenzenZuerichFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    PassantenfrequenzenZuerichUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = PassantenfrequenzenZuerichHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = PassantenfrequenzenZuerichHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = PassantenfrequenzenZuerichHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => PassantenfrequenzenZuerichError.new(
        "#{op}_allow",
  ...
  def raw_request(fetchargs = {})
    ...
    rescue PassantenfrequenzenZuerichError => err
    ...
    if fetched.is_a?(Hash)
      status = PassantenfrequenzenZuerichHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = PassantenfrequenzenZuerichError.new(
        "graphql_error", "PassantenfrequenzenZuerichSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = PassantenfrequenzenZuerichSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
