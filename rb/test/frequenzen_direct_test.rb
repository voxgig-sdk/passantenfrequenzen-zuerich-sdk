...
class FrequenzenDirectTest < Minitest::Test
  def test_direct_list_frequenzen
    setup = frequenzen_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-frequenzen", setup[:live] ? "live" : "unit")
    if _should_skip
...
def frequenzen_direct_setup(mockres)
