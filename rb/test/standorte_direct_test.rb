...
class StandorteDirectTest < Minitest::Test
  def test_direct_list_standorte
    setup = standorte_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-standorte", setup[:live] ? "live" : "unit")
    if _should_skip
...
def standorte_direct_setup(mockres)
