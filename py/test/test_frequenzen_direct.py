...
class TestFrequenzenDirect:

    def test_should_direct_list_frequenzen(self):
        setup = _frequenzen_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-frequenzen", "live" if setup["live"] else "unit")
        if _skip:
...
def _frequenzen_direct_setup(mockres):
