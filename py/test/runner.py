# PassantenfrequenzenZuerich SDK test runner
...
class PassantenfrequenzenZuerichTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            PassantenfrequenzenZuerichTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = PassantenfrequenzenZuerichTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = PassantenfrequenzenZuerichTestRunner.getenv("PASSANTENFREQUENZEN_ZUERICH_TEST_LIVE")
        override = PassantenfrequenzenZuerichTestRunner.getenv("PASSANTENFREQUENZEN_ZUERICH_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = PassantenfrequenzenZuerichTestRunner.getenv(key)
        ...
        explain = PassantenfrequenzenZuerichTestRunner.getenv("PASSANTENFREQUENZEN_ZUERICH_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["PASSANTENFREQUENZEN_ZUERICH_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if PassantenfrequenzenZuerichTestRunner._test_control is not None:
            return PassantenfrequenzenZuerichTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                PassantenfrequenzenZuerichTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            PassantenfrequenzenZuerichTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = PassantenfrequenzenZuerichTestRunner.load_test_control()
...
def load_env_local():
    PassantenfrequenzenZuerichTestRunner.load_env_local()


def env_override(m):
    return PassantenfrequenzenZuerichTestRunner.env_override(m)


def entity_data(v):
    return PassantenfrequenzenZuerichTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return PassantenfrequenzenZuerichTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return PassantenfrequenzenZuerichTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return PassantenfrequenzenZuerichTestRunner.load_test_control()


def live_client_options():
    return PassantenfrequenzenZuerichTestRunner.live_client_options()


def live_delay_ms():
    return PassantenfrequenzenZuerichTestRunner.live_delay_ms()
