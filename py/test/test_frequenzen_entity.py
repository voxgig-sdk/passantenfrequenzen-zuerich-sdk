...
class TestFrequenzenEntity:
    ...
    def test_should_stream(self):
        ...
        seen = list(base.Frequenzen(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.Frequenzen(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set PASSANTENFREQUENZEN_ZUERICH_TEST_FREQUENZEN_ENTID JSON to run live")
        client = setup["client"]
        ...
        frequenzen_ref01_list_result = frequenzen_ref01_ent.list(frequenzen_ref01_match, None)
...
def _frequenzen_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("PASSANTENFREQUENZEN_ZUERICH_TEST_FREQUENZEN_ENTID"))
