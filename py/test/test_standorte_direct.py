...
class TestStandorteDirect:

    def test_should_direct_list_standorte(self):
        setup = _standorte_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-standorte", "live" if setup["live"] else "unit")
        if _skip:
...
def _standorte_direct_setup(mockres):
