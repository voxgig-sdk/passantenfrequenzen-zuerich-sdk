# PassantenfrequenzenZuerich Python SDK



The Python SDK for the PassantenfrequenzenZuerich API — an entity-oriented client following Pythonic conventions.

The SDK exposes the API as capitalised, semantic **Entities** — for example `client.Frequenzen()` — each
...
release tag (`py/vX.Y.Z`, see [Releases](https://github.com/voxgig-sdk/passantenfrequenzen-zuerich-sdk/releases)) or
...
| --- | --- | --- |
| `age_group` | `str` | Altersgruppe |
| `count` | `int` | Anzahl gezählter Passanten |
| `direction` | `str` | Laufrichtung der Passanten |
| `location` | `str` | Name des Messgebiets |
| `temperature` | `float` | Temperatur in Grad Celsius |
| `timestamp` | `str` | Zeitpunkt der Messung in UTC (ISO 8601) |
| `weather` | `str` | Wetterbedingungen während der Messung |
...
```
py/
├── passantenfrequenzenzuerich_sdk.py         -- Main SDK module
