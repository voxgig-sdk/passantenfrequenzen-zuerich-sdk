...
//    only - a typed PassantenfrequenzenZuerichContext there would make every ctx
...
final class PassantenfrequenzenZuerichOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new PassantenfrequenzenZuerichContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new PassantenfrequenzenZuerichSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new PassantenfrequenzenZuerichResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new PassantenfrequenzenZuerichError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new PassantenfrequenzenZuerichResponse($clean['response']);
    ...
    public static function err_from_map($m): ?PassantenfrequenzenZuerichError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new PassantenfrequenzenZuerichError(is_string($code) ? $code : '', $msg);
    }
