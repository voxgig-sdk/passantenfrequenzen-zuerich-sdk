...
// PassantenfrequenzenZuerich SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = PassantenfrequenzenZuerichOmni::makeStructRunner(
    ...
    {
        return PassantenfrequenzenZuerichOmni::objmodel(PassantenfrequenzenZuerichOmni::argval($val));
    }

    /** A single-case fixture value on the comparison side. */
    private static function outval($val)
    {
        return PassantenfrequenzenZuerichOmni::unmark($val);
    }

    /** A result normalised to the comparison model. */
    private static function resval($val)
    {
        return PassantenfrequenzenZuerichOmni::structfix($val, Struct::undef());
    }
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(PassantenfrequenzenZuerichOmni::structfix($now, Struct::undef()))
                        !== json_encode(PassantenfrequenzenZuerichOmni::structfix($val, Struct::undef()))) {
