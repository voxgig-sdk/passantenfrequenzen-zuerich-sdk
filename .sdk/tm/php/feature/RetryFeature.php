...
// PassantenfrequenzenZuerich SDK retry feature
...
class PassantenfrequenzenZuerichRetryFeature extends PassantenfrequenzenZuerichBaseFeature
{
    ...
    public function init(PassantenfrequenzenZuerichContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (PassantenfrequenzenZuerichContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(PassantenfrequenzenZuerichContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(PassantenfrequenzenZuerichContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
