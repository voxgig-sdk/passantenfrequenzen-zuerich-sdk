# passantenfrequenzen-zuerich-mcp

[MCP](https://modelcontextprotocol.io) server exposing the PassantenfrequenzenZuerich SDK as
two agent tools — `passantenfrequenzen-zuerich_list` and `passantenfrequenzen-zuerich_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/passantenfrequenzen-zuerich-mcp)
make build

# 2. Provide credentials via the environment
export PASSANTENFREQUENZEN_ZUERICH_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user passantenfrequenzen-zuerich \
  -- /absolute/path/to/passantenfrequenzen-zuerich-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./passantenfrequenzen-zuerich-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// passantenfrequenzen-zuerich_list: first page of records
{ "entity": "frequenzen" }
{ "entity": "frequenzen", "query": { } }

// passantenfrequenzen-zuerich_load: one record by id
{ "entity": "frequenzen", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/passantenfrequenzen-zuerich-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user passantenfrequenzen-zuerich \
     -- "$PWD"/dist/*/passantenfrequenzen-zuerich-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `passantenfrequenzen-zuerich_list` and `passantenfrequenzen-zuerich_load` tools now appear
   in new sessions. Ask the agent to *"list frequenzen using passantenfrequenzen-zuerich"*
   and it calls `passantenfrequenzen-zuerich_list` with `{"entity":"frequenzen"}`.
...
```sh
export PASSANTENFREQUENZEN_ZUERICH_APIKEY=sk_live_xxx            # API key
export PASSANTENFREQUENZEN_ZUERICH_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `passantenfrequenzen-zuerich_list` tool
...
### Call the `passantenfrequenzen-zuerich_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `passantenfrequenzen-zuerich_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `passantenfrequenzen-zuerich_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `PASSANTENFREQUENZEN_ZUERICH_APIKEY` | API key sent with every request. |
| `PASSANTENFREQUENZEN_ZUERICH_BASE` | Optional override of the API base URL. |

### Entities

The 2 entities valid as the `entity` argument:

frequenzen | standorte

### Smoke test via HTTP (raw JSON-RPC)

```sh
./passantenfrequenzen-zuerich-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"passantenfrequenzen-zuerich_load","arguments":{"entity":"frequenzen","query":{"id":1}}}}'
```
